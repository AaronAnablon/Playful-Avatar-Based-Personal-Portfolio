...
function Projects({ title }, ref) {
  ...
  return (
    <div ref={ref}>
      ...
      <div className="h-max w-full overflow-hidden mb-40 lg:mb-0 flex items-center justify-center">
        <div className='grid grid-cols-1 gap-48'>
          {ProjectCards && ProjectCards.map((card) => (
            <div className={`grid lg:grid-cols-2 grid-cols-1 h-64`} key={card.id}>
                ...
                <div className='flex px-10'>
                  <div className='relative w-4/12'>
                    ...
                    {card.tablet && <>
                      ...
                      <Image src={card.tablet} alt='tablet' className='absolute overflow-hidden p-1 lg:p-2 md:p-4 z-0 inset-0' />
                    </>}
                  </div>

                  <div className='relative w-full'>
                    <Image src={laptop} alt='laptop' className='absolute z-10 w-full inset-0' />
                    <Image src={card.desktop} alt='desktop' className='absolute px-8 md:px-20 lg:px-14 lg:mt-4 mt-2 md:mt-6 overflow-hidden z-0 inset-0' />
                  </div>

                  <div className='relative w-2/12'>
                    {card.phone && <>
                      <Image src={phone} alt='phone' className='absolute z-10 w-full inset-0' />
                      <Image src={card.phone} alt='phone' className='absolute overflow-hidden p-0.5 md:p-1 z-0 inset-0' />
                    </>}
                  </div>
                </div>
              </motion.div>
